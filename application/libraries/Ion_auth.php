...
class Ion_auth
{
	...
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array(),$fb = FALSE) //need to test email activation
	{
		...
		if ($email_activation) {
			...
			$form = $this->config->item('site_settings')->portal_email;
			$to   = $email;
			$subject = 'Activation Email';
			$msg     = 'Please activate your account by clicking on the following link: ' . anchor(SITEURL2 . '/auth/activate/' . $id . '/' . $activation_code, 'Activate Your Account');

			if (sendEmail($form, $to, $subject, $msg)) {
